...
Test caption model inference from a loaded checkpoint.
"""
...
def infer():
    ...
    encoder.load_state_dict(
        torch.load(
            os.path.join(CKPT_PATH, "encoder-%d.pth" % 440),
    ...
    decoder.load_state_dict(
        torch.load(
            os.path.join(CKPT_PATH, "decoder-%d.pth" % 440),
            map_location=torch.device(device),
        )
    )
    encoder.eval()
...
def gt_vec_to_text(caption, dataset):
    """Convertsword vector representation of ground-truth caption to text.
